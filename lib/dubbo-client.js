 ...
 * 获取provider的对象
 */
DubboClient.prototype.getProvider = function(provider, group, version, callback) {
  ...
  var filterProvider = function(providers, callback) {
    ...
    if (len) {
      ...
      if (randomProvider.methods.length > 0) {
        ...
        callback(Error([':(, Not found exports method', provider + ', version:', version, ', group:', group].join(' ')));
      }
    } else {
      callback(Error([':(, Not found', provider + ', version:', version, ', group:', group].join(' ')));
    }
  };
