...
function ZookeeperRegistry(url, config) {
  ...
  this._client.once('connected', function () {
    //连接上之后，检测是不是在消息队列中包含消息
    Object.keys(this._msgLoop).forEach(function (provider) {
      self._syncProvider(provider, function (err, providerList) {
        self._client.emit(provider, err, providerList);
        delete self._msgLoop[provider];
      });
    });


    //监听新的接口请求，此刻有种erlang的感觉。恩。消息驱动，牛叉。
    self._client.on('newProvider', function (provider) {
      self._syncProvider(provider, function (err, providerList) {
        self._client.emit(provider, err, providerList);
        delete self._msgLoop[provider];
      });
...
ZookeeperRegistry.prototype.getProvider = function(provider, callback) {
  ...
  if (!this._msgLoop[provider]) {
    this._client.emit('newProvider', provider);
  }
  this._client.once(provider, callback);
};
...
ZookeeperRegistry.prototype._syncProvider = function(provider, callback) {
  ...
  //cache,优化点，当多个相同的服务请求,请求一个，然后回调，解决雪崩
  this._proxy.once(provider, callback);
