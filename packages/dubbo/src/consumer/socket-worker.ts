...
export default class SocketWorker implements IObservable<ISocketSubscriber> {
  ...
  private _initSocket() {
    ...
    this._socket
      ...
      .on('data', () => {
        log(
          `socket-worker#${this.pid}  =receive data=> ${this.host}:${this.port}`,
        )
      })
  ...
  private _onError = (error: Error) => {
    log(
      `socket-worker#${this.pid} <=occur error=> ${this.host}:${this.port} ${error}`,
    )
  }

  private _onClose = (hadError: boolean) => {
    log(
      `socket-worker#${this.pid} <=closed=> ${this.host}:${this.port} hasError: ${hadError} retry: ${this._retry}`,
    )
