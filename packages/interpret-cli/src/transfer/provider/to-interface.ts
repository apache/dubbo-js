import {InterfaceDeclarationStructure, MethodSignatureStructure, PropertySignatureStructure} from "ts-simple-ast";
...
export async function toInterface(typeDef: IJClass, intepretHandle: IntepretHandle): Promise<InterfaceDeclarationStructure> {
  ...
  for (let fieldName in typeDef.fields) {
    if (typeDef.fields[fieldName].name.startsWith("com.qianmi") || typeDef.fields[fieldName].name.startsWith("java.")) {
      properties.push(await toField(fieldName, typeDef.fields[fieldName], intepretHandle));
    }
  }
  ...
  for (let methodName in typeDef.methods) {
    ...
    methods.push(await toMethod(methodName, typeDef.methods[methodName], intepretHandle));
  }
