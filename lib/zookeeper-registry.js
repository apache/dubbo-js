...
registry.init = function (url, config) {
  ...
  // 创建zk的客户端连接对象
  zkClient = zookeeper.createClient(url, config['zk']);
...
registry.getProviderMeta = function(provider, group, version) {
  ...
  var syncProvider = function(resolve, reject, provider) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (providerList) {
      ...
      //如果还没有连接
      if (zkClient == null || zkClient['state']['name'] !== 'SYNC_CONNECTED') {
        //waiting zk connected.
        zkClient.once(READY, function () {
          syncProvider(resolve, reject, provider);
        });
      } else {
        syncProvider(resolve, reject, provider);
      }
